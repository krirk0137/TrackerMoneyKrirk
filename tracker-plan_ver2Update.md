...
│   ├── 03_seed_trigger.sql
│   ├── 04_functions.sql
│   └── 05_grants.sql        # GRANT ให้ role (จำเป็นเพราะปิด auto-expose)
├── .github/workflows/
│   ├── keep-alive.yml
│   └── backup.yml
└── README.md
...
-- ฟังก์ชันเบาๆ สำหรับ keep-alive (ให้ anon เรียกได้ ไม่แตะข้อมูลจริง)
...
## 8.1 Grants — `sql/05_grants.sql` (สำคัญ: รันเป็นไฟล์สุดท้าย)

เพราะปิด **Automatically expose new tables** ในตอนสร้างโปรเจกต์ ตารางจะเข้าถึงผ่าน supabase-js ไม่ได้จนกว่าจะ GRANT เอง (ไม่งั้นเจอ error `42501 permission denied for table`) แอปนี้ login อย่างเดียว → ให้สิทธิ์เฉพาะ role `authenticated`

```sql
-- ให้ role authenticated ใช้ schema + เข้าถึงตารางของแอป
grant usage on schema public to authenticated;
grant select, insert, update, delete on public.categories   to authenticated;
grant select, insert, update, delete on public.transactions to authenticated;

-- ให้ authenticated เรียก RPC สรุปยอดได้
grant execute on function public.monthly_summary(int, int) to authenticated;

-- ให้ anon เรียกได้เฉพาะ keep_alive (สำหรับ GitHub Actions กัน pause) — ไม่แตะข้อมูลจริง
grant execute on function public.keep_alive() to anon;
```

> หมายเหตุ: ไม่ต้อง grant อะไรให้ `anon` บนตาราง categories/transactions เลย — anon เข้าถึงข้อมูลการเงินไม่ได้ตั้งแต่ชั้น grant (เป็น defense-in-depth ทับ RLS อีกชั้น)
...
โปรเจกต์ฟรีจะถูก pause ถ้าไม่มี request เข้า DB ครบ 7 วัน → เรียก RPC `keep_alive` (ที่ grant ให้ anon) เบาๆ ทุก 3 วันกันไว้
...
> ใช้ RPC แทนการ query ตารางตรงๆ เพราะ anon ไม่มีสิทธิ์อ่าน categories/transactions แล้ว (grant ให้แค่ `authenticated`) — `keep_alive()` แค่รัน `select 'ok'` ไม่แตะข้อมูล
...
- สร้างโปรเจกต์ Supabase ตั้งค่า Data API ตอนสร้าง:
  - Enable Data API → **ON**
  - Automatically expose new tables → **OFF** (คุมสิทธิ์ด้วย GRANT เอง)
  - Enable automatic RLS → **ON**
- รัน SQL ตามลำดับ `sql/01 → 02 → 03 → 04 → 05` (05_grants ต้องรันสุดท้าย เพราะ grant execute บนฟังก์ชันใน 04)
- ตรวจว่า RLS เปิดครบทุกตาราง และทดสอบว่า supabase-js query ตารางได้ (ไม่เจอ error 42501)
...
6. **GRANT กับ RLS เป็นคนละชั้น** — grant คุมว่า role เข้าถึงตารางได้ไหม, RLS คุมว่าเห็นแถวไหน ให้ grant เฉพาะ `authenticated` บนตารางข้อมูล ไม่ให้ `anon` แตะตารางข้อมูลเลย
...
**การตั้งค่า Data API (ตรวจ มิ.ย. 2026):** ตั้งแต่ 30 พ.ค. 2026 โปรเจกต์ใหม่ของ Supabase ไม่ auto-grant สิทธิ์ให้ตารางใน public schema อีกต่อไป ทุกตารางต้อง `GRANT` เองจึงจะเข้าถึงผ่าน supabase-js/PostgREST/GraphQL ได้ ถ้าขาด grant จะได้ error `42501 permission denied for table` พร้อม hint ให้ GRANT — พฤติกรรมนี้อาจปรับเพิ่มเติมได้ ให้ตรวจ changelog ของ Supabase ก่อนตั้งค่าจริง
